...
@synthesize albumArtiste, albumAlbum, albumPid,emailUser, databasePath;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // Configure the cell...
	cell.primaryLabel.text = [self.albumAlbum objectAtIndex:indexPath.row];
	cell.secondaryLabel.text = [self.albumArtiste objectAtIndex:indexPath.row];
	
	UIImage *image;
	NSString *path = [[NSBundle mainBundle] bundlePath];
	NSData *data = [NSData dataWithContentsOfFile:[NSString stringWithFormat:@"%@/albums.png",path]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if(sqlite3_open([databasePath UTF8String], &database) == SQLITE_OK) {
		NSString *pidAlbum = [NSString stringWithFormat:@"%@",[self.albumPid objectAtIndex:indexPath.row]];
		
		NSString *requete = @"";
		if(verifios5){
			requete = [NSString stringWithFormat:@"SELECT it.item_pid, ite.title, ita.item_artist, a.album, ite.total_time_ms,ite.comment, ite.grouping FROM item it, item_extra ite, item_artist ita, album a WHERE it.item_pid=ite.item_pid AND it.item_artist_pid=ita.item_artist_pid AND it.album_pid=a.album_pid AND it.album_pid='%@' ORDER BY ite.title COLLATE NOCASE",pidAlbum];
		}
		else{
			requete = [NSString stringWithFormat:@"SELECT pid, title, artist, album,total_time_ms, comment, grouping FROM item WHERE album_pid='%@' ORDER BY track_number",pidAlbum];
		}
	...
	detailViewController.isAlbum = @"album";
	
	detailViewController.navigationItem.title = [self.albumAlbum objectAtIndex:indexPath.row];
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
	...
	if (buttonIndex == 1) {
		...
		for (int i=0; i<[self.albumAlbum count]; i++) {
			
			exportContents = [exportContents stringByAppendingString:[NSString stringWithFormat:@"<p><b>%@</b> - %@</p>",[self.albumAlbum objectAtIndex:i],[self.albumArtiste objectAtIndex:i]]];
		}
