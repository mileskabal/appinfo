...
@synthesize dicoApple;
@synthesize appDetailViewController;
@synthesize unitUser;
@synthesize aWebView;
@synthesize appleTableView;
...
- (void)viewDidLoad {
	...
	NSArray *files;
	NSString *file;
	dicoApple = [[NSMutableDictionary alloc] init];
	
	
	documentsDir = delegate.pathAppSystem;
	
	//files = [[NSFileManager defaultManager] directoryContentsAtPath:documentsDir];
	...
	for(file in files){
		...
		if([[NSFileManager defaultManager] fileExistsAtPath:infoPlistPath] == YES){			
			...
			if([CFBundleIdentifier hasPrefix:@"com.apple"]){
				[dicoApple setObject:app forKey:[NSString stringWithFormat:@"%@",CFBundleDisplayName]];
			}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // Set up the cell...
	id value;
	NSArray *nomApp = [dicoApple allKeys];
	NSArray *nomAppAlpha = [nomApp sortedArrayUsingSelector:@selector(caseInsensitiveCompare:)];
	value = [dicoApple objectForKey:[nomAppAlpha objectAtIndex:indexPath.row]];
			...
			if([[NSFileManager defaultManager] fileExistsAtPath:imageReTest] == YES){
				...
				if([[NSFileManager defaultManager] fileExistsAtPath:imageTT] == YES){
					...
					if([donnees_nom compare:[NSString stringWithFormat:@"MobileMusicPlayer"]] == NSOrderedSame){
						...
						NSData *data = [NSData dataWithContentsOfFile:[NSString stringWithFormat:@"%@/apple.png",path]];
						image = [[UIImage alloc] initWithData:data];
						cell.myImageView.image = image;
...
- (void)detailDiscolosureIndicatorSelected:(UIButton *)sender  {  
	...
	id value = [dicoApple objectForKey:[nomAppAlpha objectAtIndex:sender.tag]];	
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	NSString *bundleInfo = [NSString stringWithFormat:@"<b>%@ :</b> %@<br /><b>%@ :</b> %@<br /><b>%@ :</b> %@<br /><b>%@ :</b> %@<br />%@", MyLocalizedString(@"_version",@""), versionApplication, MyLocalizedString(@"_dossier",@""), dossierApplication, MyLocalizedString(@"_minos",@""),minosApplication,MyLocalizedString(@"_identifiant",@""), identifiantApplication,posInSB];
	...
	//[delegate.appleNavController pushViewController:appDetailViewController animated:YES];
...
- (BOOL)webView:(UIWebView*)webView shouldStartLoadWithRequest:(NSURLRequest*)request navigationType:(UIWebViewNavigationType)navigationType {
	...
	if(navigationType == UIWebViewNavigationTypeLinkClicked) {
		if([urlString hasPrefix:@"ifile"] || [urlString hasPrefix:@"http"] || [urlString hasPrefix:@"cydia"]){
			...
			id value;
			value = [dicoApple objectForKey:keyID];
